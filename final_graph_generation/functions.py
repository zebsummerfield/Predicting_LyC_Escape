...
def prepare_data(file, f_or_n=0, obvs=False, obvs_cat='charlotte', eps=True, add_vars=[]):
    """
...
def prepare_data_dusty(file, f_or_n=0, obvs=False, obvs_cat='charlotte', eps=True, add_vars=[]):
    """
    Loads the data from the dusty training catalogue and prepares it for model training
    ...
    with h5py.File(file, 'r') as hdf:
        ...
        vir_mass = np.array(hdf['M_vir_full'])
        gas_mass = np.array(hdf['gas_mass_full'])
        ha_lum = np.array(hdf['ha_lum_obs_full']) * 5
        ...
        uv_int_mag = -2.5 * np.log10((f_esc_vars[8]) / lum_to_tenpc) - 48.6
