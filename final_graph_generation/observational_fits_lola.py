...
file3 = folder + 'f_esc_rf_observational_lola_test_train.json'
file4 = folder + 'n_esc_rf_observational_lola_test_train.json'
...
with open('lola_cat.txt', 'r') as f:
    ...
    obvs_f_esc_vars = [obvs_ssfr10, obvs_ssfr100, obvs_ssfr10/obvs_ssfr100, obvs_star_mass, obvs_uv_flux,
                    obvs_uv_flux/obvs_ha_flux, obvs_uv_size, obvs_ha_size, 1+obvs_redshift, random_variable]
    obvs_f_esc_keys = ['offset10', 'ssfr100', 'ssfr10/ssfr100', 'star_mass', 'uv_mag',
    ...
    obvs_log_n_esc_vars[4] = obvs_ha_mag.astype('float32')
...
for i_1 in range(len(axes)):
    ...
    for i_2 in range(len(axes)):
        ...
        loaded_model = joblib.load(folder + ['f_esc', 'n_esc'][i_2] + '_rf_observational_lola_model.pkl')
        ...
        scatter = axes[i_2][i_1].scatter(x_var[sorted_indices], predictions[sorted_indices], alpha=0.9,
                                         c=z[sorted_indices], cmap='inferno', s=30, vmin=4, vmax=6, zorder=3)

        y_range = ([-2.5, -0.5], [51, 54])[i_2]
