...
if f_or_n == 0:
    file = 'f_esc_rf_final_test_train.json'
else:
    file = 'n_esc_rf_final_test_train.json'

with open(folder+file, 'r') as json_data:
    f_data = json.load(json_data)
    esc_test = np.array(f_data['f_esc_test'])
    esc_train = np.array(f_data['f_esc_train'])
    esc_test_pred = np.array(f_data['f_esc_test_pred'])
    esc_train_pred = np.array(f_data['f_esc_train_pred'])
