...
with h5py.File(file, 'r') as hdf:
    ...
    n_esc = np.array(hdf['Ndot_LyC_vir_full']) / f_esc
    ...
    bad_indices = []
    print(f"non-centre halo galaxy bad rows: {len(bad_indices)}")
...
with open(folder+f'{f_or_n_str}_rf_{obvs_str}_test_train{testing}.json', 'w') as f:
...
if obvs:
    joblib.dump(best_rf, folder+f'{f_or_n_str}_rf_{obvs_str}_model{testing}.pkl')
